  ...
  <div id="main">
    <div class="content page-order-payment">
      <div class="gray-box clear">
        ...
        <div class="box-inner payment-checkout-panel clear">
	    	            <span class="jianguo-blue-main-btn big-main-btn js-payment-order"
                          @click="payNow(orderInfo.orderId)" v-if="orderInfo.isPay">
                      <a>现在支付</a>
	    	            </span>
          <span v-else class="is-pay">已经完成</span>
      ...
      <div class="confirm-detail">
        ...
        <p class="info-detail">{{orderInfo.receiverInfo.province}} {{orderInfo.receiverInfo.city}}
          {{orderInfo.receiverInfo.county}} {{orderInfo.receiverInfo.add}}</p>
      </div>
      <div class="confirm-detail">
...
<script>
  export default {
    ...
    computed: {
      orderInfo () {
        let orderInfo =
          this.$store.state.orderData.filter((order) => {
