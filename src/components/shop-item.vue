...
<template>
  <div class="item">
    <div>
      <div class="item-img"><img :alt="item.name_title"
      ...
      <div class="params-colors">
        <ul class="colors-list">
          <li v-for="(items , index) in item.sku_info">
            <a href="javascript:;"
               :class="{'active':index==itemIndex}"
               :title="items.spec_json.show_name"
               @click="tableData(index)"
            >
              <img :src="'http://img01.smartisanos.cn/'+items.spec_json.image+'20X20.jpg'"></a>
          </li>
        </ul>
      </div>
      <div class="item-btns clearfix">
        <span class="item-gray-btn"> <router-link
          v-bind:to="{name:'Item',query:{itemId:item.sku_info[itemIndex].sku_id}}">查看详情</router-link> </span><span
        class="item-blue-btn" @click="addCarPanelData(item.sku_info[itemIndex])">加入购物车 </span>
      </div>
      ...
      <div class="item-cover">
        <router-link v-bind:to="{name:'Item',query:{itemId:item.sku_info[itemIndex].sku_id}}"></router-link>
      </div>
    </div>
  </div>
</template>
